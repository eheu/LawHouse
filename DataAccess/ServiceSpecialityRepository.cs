...
using System.Data;
using Abstraction.ServiceSpecialities;
...
namespace DataAccess
{
    public class ServiceSpecialityRepository : IServiceSpecialityRepository
    {
        ...
        /// <summary>
        ///     Create a new instance of <see cref="ServiceSpecialityRepository" />.
        /// </summary>
        /// <param name="transaction">Active transaction</param>
        /// <exception cref="ArgumentNullException">transaction</exception>
        public ServiceSpecialityRepository(SqlTransaction transaction)
        {
        ...
        public void Create(ServiceSpeciality entity)
        {
            ...
            try
            {
                ...
                cmd.CommandText = @"INSERT INTO ServiceSpeciality (serviceID, specialityID)
        ...
        public ServiceSpeciality Get(int ID)
        {
            ...
            {
                cmd.Transaction = _transaction;
                cmd.CommandText = @"SELECT serviceID, specialityID
                                    FROM ServiceSpeciality
                                    WHERE serviceID = @serviceID";
                cmd.AddParameter("serviceID", ID);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        var entity = new ServiceSpeciality();
                        Map(reader, entity);
                        return entity;
                    }
                    return null;
        ...
        public void Update(ServiceSpeciality entity)
        {
            ...
            {
                cmd.Transaction = _transaction;
                cmd.CommandText = @"UPDATE ServiceSpeciality S
                                    WHERE serviceID = @serviceID";
                cmd.AddParameter("serviceID", entity.ServiceID);
                cmd.AddParameter("specialityID", entity.SpecialityID);
        ...
        /// <exception cref="ArgumentOutOfRangeException">Id is less than 1.</exception>
        /// <exception cref="DataException">Db operation failed</exception>
        public List<ServiceSpeciality> FindAll()
        {
            var cmd = _transaction.Connection.CreateCommand();
            try
            {
        ...
        private static void Map(IDataRecord record, ServiceSpeciality dto)
        {
            dto.ServiceID = (int)record[0];
        ...
        private static List<ServiceSpeciality> MapCollection(SqlCommand cmd)
        {
            using (var reader = cmd.ExecuteReader())
            {
                var items = new List<ServiceSpeciality>();
                while (reader.Read())
                {
                    var item = new ServiceSpeciality();
                    Map(reader, item);
                    items.Add(item);
                }
                return items;
